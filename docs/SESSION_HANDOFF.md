...
- Date: 2026-07-15.
- Branch: `feature/F133-files-preview-size-cap` at `0172fb2`, based on `main` through merged PR
  #146 (F132).
- Queue: F114-F121, F128, F129, F130, F131, F132, F134, F139, and F140 are `passing` (merged);
  F133 is `passing` (pending this branch's PR); F135-F138 and F141-F143 are `not_started`
...
- A deep read-only code review of the product workspace (2026-07-14) produced
  ...
  (PR #140), F130 (#141), F134 (#142), F139 (#143), F140 (#144), F131 (#145), and F132 (#146) are
  merged: the session-token handout is documented as an accepted local-trust limitation (S1);
  `--verbose` logs mask the export URL's token (S2/C2); combined column rename+alter is atomic on
  Postgres/SQLite with an honest partial-success result on MySQL (V1); the SQL Editor's
  write-attempt errors now trigger the F120 capability-cache refresh (U1); `EditableCell`/
  `NewRowCell` gained NULL/empty-string reachability, Escape-to-cancel on date editors, and
  bigint-precision rejection (U2/U4/U5); `describeError()` redacts credentials embedded in driver
  error text (S3); and recent-targets persistence catches compound credential-param names (S4).
  See each id's evidence in `docs/FEATURES.json` for full reasoning/test detail.
- F133 (`0172fb2`, pending PR): closed SUGGESTIONS.md S5 - `GET /api/files/content` now caps a
  preview read at `FILES_PREVIEW_MAX_BYTES` (1 MiB) via a bounded `readSync` instead of loading
  the whole file, with `FilesBrowser` showing an explicit truncation notice and hiding "Run in
  editor" for a truncated preview. See `docs/FEATURES.json`'s F133 evidence for live-verification
  detail (curl against the running server, since the SQLite connect UI wasn't easily automatable
  in this session).
...
- Open/merge F133's PR, then promote the next task from the F135-F138/F141-F143 review-fix queue
  (see `docs/SUGGESTIONS.md` for each finding's full context). Suggested order: remaining minors
  (F135-F138, F141, F142); F143 (scalable-structure refactor) last so file moves don't conflict
