...
- Frontend: React + Vite + TanStack Router/Query + Tailwind + a shadcn-style `@humbdb/ui` kit.
- Postgres driver via `pg` in `packages/drivers/postgres` (`@humbdb/postgres`), behind a generic
  `DatabaseAdapter` contract in `packages/drivers/contract` (`@humbdb/driver-contract`).
- New engines are additive `packages/drivers/<engine>` packages, named `@humbdb/<engine>` (no `db-`
...
- 2026-07-01: renamed `packages/db-adapter` -> `packages/drivers/contract` (`@humbdb/driver-contract`)
  and `packages/db-postgres` -> `packages/drivers/postgres` (`@humbdb/postgres`), and required a
