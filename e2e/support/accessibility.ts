...
export async function expectNoAccessibilityViolations(page: Page, surface: string): Promise<void> {
  ...
  try {
    for (const theme of THEMES) {
      ...
      // Color utilities transition for 150ms; scan the settled theme rather than an in-between
      // frame where a light surface can briefly retain its dark foreground (or vice versa).
