...
describe("SqliteAdapter integration", () => {
  ...
  beforeAll(async () => {
    const dir = mkdtempSync(join(tmpdir(), "humb-sqlite-"));
    dbPath = join(dir, "fixture.db");

    const setup = new Database(dbPath);
    setup.exec(`
      CREATE TABLE humb_demo_users (
  ...
  it("lists tables under a single 'main' schema, excluding sqlite's internal tables", async () => {
    ...
    expect(mainSchema?.tables).toEqual(
      expect.arrayContaining(["humb_demo_users", "humb_demo_empty", "humb_demo_composite"])
    );
  ...
  it("marks a composite primary key's auto-index as primary, without an integer-rowid PK needing one", async () => {
    const composite = await adapter.getTable("main", "humb_demo_composite");
    ...
    // humb_demo_users' `id INTEGER PRIMARY KEY` is a rowid alias - SQLite creates no separate
    // index entry for it (unlike the composite case above), yet isPrimaryKey must still be true.
    const users = await adapter.getTable("main", "humb_demo_users");
  ...
  it("returns correct columns even when a table/query has zero rows", async () => {
    const table = await adapter.getTable("main", "humb_demo_empty");
    expect(table.columns.map((column) => column.name)).toEqual(["id", "note"]);
    expect(table.rowCount).toBe(0);

    const page = await adapter.getRows("main", "humb_demo_empty", 0, 10);
    expect(page.rows).toEqual([]);
    expect(page.columns).toEqual(["id", "note"]);
  });

  it("returns a page of rows", async () => {
    const page = await adapter.getRows("main", "humb_demo_users", 0, 10);
    expect(page.rows).toHaveLength(3);
    expect(page.columns).toEqual(["id", "name", "email"]);
  });

  it("runs a read-only query", async () => {
    const result = await adapter.runReadOnlyQuery("SELECT * FROM humb_demo_users");
    expect(result.rows).toHaveLength(3);
  });

  it("rejects a mutating query before it ever reaches the database", async () => {
    await expect(adapter.runReadOnlyQuery("DELETE FROM humb_demo_users")).rejects.toThrow();

    const after = await adapter.getRows("main", "humb_demo_users", 0, 10);
  ...
  it("the underlying connection is opened read-only - the authoritative backstop, not just assertReadOnly's string scan", () => {
    ...
    try {
      expect(() => directHandle.exec("DELETE FROM humb_demo_users")).toThrow(/readonly database/);
    } finally {
      directHandle.close();
    }
  });
