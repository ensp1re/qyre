...
src/routes/<domain>/<resource>.ts
                              route registrars grouped under admin, connection, data, observability, transfer, or write
...
global handler. `routes/write/schema-ddl.ts` composes separate table, column, and index route groups
under `routes/write/schema-ddl/`.
