 ...
 *   - "mysql" project: requires HUMB_TEST_MYSQL_URL (F014).
 *   All projects run every spec (including @smoke), since @humbdb/web build's built once up front
...
export default defineConfig({
  ...
  webServer: [
    ...
    },
    {
      ...
      reuseExistingServer: !process.env.CI,
      timeout: 120_000,
      // HUMB_TEST_MYSQL_URL is inherited from the parent shell environment, same as
      // HUMB_TEST_DATABASE_URL for the "postgres" project above.
      env: {
        HUMB_E2E_PORT: "4177",
        HUMB_E2E_ENGINE: "mysql"
      }
    }
  ]
});
